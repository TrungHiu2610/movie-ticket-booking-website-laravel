    ...
    <div class="row">
        <div class="col-md-10 mx-auto">
            <div class="card shadow-sm">
                    ...
                    <form action="{{ route('admin.movies.update', $movie) }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="row g-3">
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="number" class="form-control @error('duration_minutes') is-invalid @enderror"
                                    ...
                                    value="{{ old('duration_minutes', $movie->duration_minutes) }}" min="1" required>
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="number" class="form-control @error('base_price') is-invalid @enderror"
                                    id="base_price" name="base_price" value="{{ old('base_price', $movie->base_price) }}"
                            ...
                            <!-- Poster -->
                            <div class="col-md-6">
                                <label for="poster_url" class="form-label">Poster (tùy chọn - để trống nếu không
                                    đổi)</label>
                                @if ($movie->poster_url)
                                    <div class="mb-2">
                                        <img src="{{ $movie->poster_url }}" alt="{{ $movie->title }}" class="img-thumbnail"
                                ...
                                <input type="file" class="form-control @error('poster_url') is-invalid @enderror"
                                    id="poster_url" name="poster_url" accept="image/*">
                            ...
                            <div class="col-md-6">
                                ...
                                <div id="trailer_url_input">
                                    <input type="text" class="form-control @error('trailer_url') is-invalid @enderror"
                                        id="trailer_url_text" name="trailer_url"
                                ...
                                @if ($movie->trailer_url)
                                    <div class="mt-2 p-2 bg-light rounded">
                                        <small class="text-muted">Trailer hiện tại:</small><br>
                                        <small class="text-break">{{ Str::limit($movie->trailer_url, 60) }}</small>
                                    </div>
                                @endif
                            </div>
                            @error('trailer_url')
                        ...
                        <div class="col-md-4">
                            ...
                            <select class="form-select @error('age_rating') is-invalid @enderror" id="age_rating"
                                ...
                                <option value="P"
                                    {{ old('age_rating', $movie->age_rating) == 'P' ? 'selected' : '' }}>P - Phổ biến
                                </option>
                                <option value="C13"
                                    {{ old('age_rating', $movie->age_rating) == 'C13' ? 'selected' : '' }}>C13 - Trên 13
                                    tuổi</option>
                                <option value="C16"
                                    {{ old('age_rating', $movie->age_rating) == 'C16' ? 'selected' : '' }}>C16 - Trên 16
                                    tuổi</option>
                                <option value="C18"
                                    {{ old('age_rating', $movie->age_rating) == 'C18' ? 'selected' : '' }}>C18 - Trên 18
                                    tuổi</option>
                            </select>
                            @error('age_rating')
                                <div class="invalid-feedback">{{ $message }}</div>
                        ...
                        <div class="col-md-4">
                            ...
                            <select class="form-select @error('status') is-invalid @enderror" id="status"
                                ...
                                <option value="coming_soon"
                                    {{ old('status', $movie->status) == 'coming_soon' ? 'selected' : '' }}>Sắp chiếu
                                </option>
                                <option value="now_showing"
                                    {{ old('status', $movie->status) == 'now_showing' ? 'selected' : '' }}>Đang chiếu
                                </option>
                            </select>
                            @error('status')
                                <div class="invalid-feedback">{{ $message }}</div>
                ...
                <form id="delete-form" action="{{ route('admin.movies.destroy', $movie) }}" method="POST"
                    class="d-none">
                    @csrf
                    @method('DELETE')
                </form>
