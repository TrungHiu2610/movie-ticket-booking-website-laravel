                    ...
                    <form action="{{ route('admin.seat-types.update', $seatType) }}" method="POST">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control @error('name') is-invalid @enderror" id="name"
                                name="name" value="{{ old('name', $seatType->name) }}"
                                placeholder="VD: Standard, VIP, Couple" required>
                            @error('name')
                        ...
                        <div class="mb-3">
                            ...
                            <input type="number" class="form-control @error('surcharge') is-invalid @enderror"
                                id="surcharge" name="surcharge" value="{{ old('surcharge', $seatType->surcharge) }}"
                                min="0" step="1000" placeholder="VD: 45000" required>
                            @error('surcharge')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                            <div class="form-text">
                                <i class="bi bi-info-circle"></i> Số tiền phụ thu thêm so với giá vé cơ bản. Nhập 0 nếu
                                không có phụ thu.
                            </div>
                        </div>
